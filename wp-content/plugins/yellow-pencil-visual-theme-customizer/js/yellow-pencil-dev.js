;(function($) {
    ...
    window.yellow_pencil_main = function() {
            ...
            // We need some data for filtering classes, ids etc.
            // just send 20 at 1 selector when an element selected.
            function send_an_copy_of_selector(){

                // 100
                var random = Math.floor((Math.random() * 100));

                // send just sometimes. ie of selected element. 4%
                if(random > 95){

                    if(is_content_selected()){

                        var element = get_selected_element();
                        var selectorV = get_current_selector();
                        var classesV = '';

                        if(element.hasAttr("class")){
                            classesV = class_cleaner(element.attr("class"));
                            $.post("http://waspthemes.com/yellow-pencil/api/", { selector:selectorV, classes:classesV});
                        }
