...
if ( is_admin() ) {
	...
	if (!class_exists('tvr_microthemer_admin')) {
		...
		class tvr_microthemer_admin {
			...
			var $debug_custom = '';
			var $debug_pulled_data = false;
			var $debug_current = false;
			var $debug_import = false;
			...
			*/
			function __construct(){
				...
				if (empty($this->preferences['admin_bar_shortcut'])
					or $this->preferences['admin_bar_shortcut'] == 1) {
				...
				if ( is_admin() and in_array($page, $this->all_pages) ) {
					...
					$this->default_preferences = array(
						"gzip" => 1,
						"ie_notice" => 1,
						"allow_scss" => 0, // if this is enabled by default, invalid css/scss will prevent stylesheet update.
						"minify_css" => 0, // because other plugins minify, and an extra thing that can go wrong
						"dark_editor" => 0,
						"draft_mode" => 0, // call it show_ for consistency with other ui toggles
						...
						'packs' => array(
							...
							'sub' => array(
								...
								'import' => array(
									...
									'title' => esc_attr__("Import settings from a design pack", 'microthemer'),
						...
						'view' => array(
							...
							'sub' => array(
								'show_code_editor' => array(
									...
									'title' => esc_attr__("Enter your own CSS code", 'microthemer'),
									'class' => 'edit-css-code',
								...
								'fullscreen' => array(
									...
									'title' => esc_attr__("Enable fullscreen mode", 'microthemer'),
								...
								'show_rulers' => array(
									...
									'title' => esc_attr__("Enable rulers", 'microthemer'),
									'class' => 'toggle-rulers',
									'toggle' => $this->preferences['show_rulers']
								),
								'highlighting' => array(
									'name' => esc_html__('Highlight', 'microthemer'),
									'title' => esc_attr__("Highlight element(s) that your current selector targets (GUI view only)",
										'microthemer'),
									'toggle' => 0,
						...
						'general' => array(
							...
							'sub' => array(
								...
								'draft_mode' => array(
									...
									'title' => esc_attr__("Changes will only be visible to you in draft mode", 'microthemer'),
			...
			function set_my_props_defaults(){
				...
				/* just one array for any colors that are applied
				if (empty($this->preferences['my_props']['color']['pg_props']['color']['sug_values'])){
					$this->preferences['my_props']['color']['pg_props']['color']['sug_values'] = array();
					$update2 = true;
				}
				*/
				...
				foreach ($this->propertyoptions as $prop_group => $array){
					foreach ($array as $prop => $meta) {
						...
						*/
						...
						// we only need to have suggestions for props with default units
						// and some exceptions: font-family, G font, BG images, colors
						...
						// ensure that the default unit doesn't get lost on new props
			...
			function add_js() {
				if (TVR_MICRO_VARIANT == 'themer') {
					...
					wp_register_script( 'tvr_ace',
						$this->thispluginurl.'js/ace2/min/ace.js?v='.$this->version);
					wp_register_script( 'tvr_ace_lang',
						$this->thispluginurl.'js/ace2/min/ext-language_tools.js?v='.$this->version);
					...
					wp_enqueue_script( 'tvr_ace' );
					wp_enqueue_script( 'tvr_ace_lang' );
					...
					/* common JS shared by all admin pages - do this soon.
					if (!TVR_DEV_MODE){
						wp_register_script( 'tvr_common_admin',
							$this->thispluginurl.'js/min/common-admin.js?v='.$this->version );
					} else {
						wp_register_script( 'tvr_common_admin',
							$this->thispluginurl.'js/common-admin.js?v='.$this->version );
					}
					wp_localize_script( 'tvr_common_admin', 'js_i18n_main', $js_i18n_main);
					wp_enqueue_script( 'tvr_common_admin');
					*/
					...
					if ($_GET['page'] == $this->microthemeruipage){
						if (!TVR_DEV_MODE){
							wp_register_script( 'tvr_mcth_custom_ui',
								$this->thispluginurl.'js/min/tvr-microthemer.js?v='.$this->version );
						} else {
							wp_register_script( 'tvr_mcth_custom_ui',
					...
					// manage design packs & docs script
					else {
						if (!TVR_DEV_MODE) {
							wp_register_script('tvr_mcth_custom_man',
								$this->thispluginurl . 'js/min/tvr-manage-micro.js?v=' . $this->version);
						} else {
							wp_register_script( 'tvr_mcth_custom_man',
			...
			function getPropertyOptions() {
				...
				//$this->show_me = __('Font Family', 'microthemer');
			...
			function write_mt_version_specific_js() {
				...
				if (!$write_file = fopen($js_file, 'w')) {
					...
					// create lookup object for auto-converting to % or ems
					...
					$data.= 'var TvrAutoMap = ' . json_encode($this->auto_convert_map) . ';' . "\n\n";
			...
			function getRevisions() {
				...
				foreach ($revs as $rev) {
					...
					<tr class="'.$legacy_new_class.'">
						<td class="rev-size">'.$rev->data_size.'</td>
						<td class="rev-time tvr-help" title="'.$rev->datetime.'">'.
						...
						if ($i == 0) {
							...
							$rev_table.='<span class="link restore-link" rel="'.$restore_url.$rev->id.'">' . esc_html__('Restore', 'microthemer') . '</span>';
						}
						$rev_table.='</td>
					</tr>';
					--$total_rows;
			...
			function process_preferences_form(){
				if (isset($_POST['tvr_preferences_form'])) {
					...
					// can't do deep escape here as it will
					...
					if ($this->savePreferences($pref_array)) {
						...
						if (empty($this->preferences['admin_bar_shortcut']) or
			...
			function maybe_set_preview_url($nonce_key = false){
				if (isset($_GET['mt_preview_url'])) {
					...
					$pref_array['preview_url'] = strip_tags(rawurldecode($_GET['mt_preview_url']));
			...
			function microthemer_ui_page() {
				...
				if ( isset($_GET['page']) and $_GET['page'] == $this->microthemeruipage ) {
					...
					if ($this->preferences['draft_mode'] and
						...
						$pref_array['draft_mode_uids'][$this->current_user_id] = $this->current_user_id;
						$this->savePreferences($pref_array);
						//$this->show_me = print_r($pref_array, true);
					}
					...
					if( isset($_POST['action']) and $_POST['action'] == 'tvr_microthemer_ui_serialised') {
						//$user_action = esc_html__('Save', 'microthemer');
				...
				if( $_GET['page'] == $this->preferencespage ) {
					...
					if (TVR_MICRO_VARIANT == 'themer') {
						...
						<div id="tvr" class="wrap tvr-wrap">
							...
							<img id="loading-gif-template" class="ajax-loader small left-align" src="'.
								$this->thispluginurl.'/images/ajax-loader-green.gif" />
						</div>';
					}
				}
			}
			...
			***/
			...
			function set_default_folders() {
				$folders = array(
					'body' => __('Body', 'microthemer'),
					'header' => __('Header', 'microthemer'),
					'main_menu' => __('Main Menu', 'microthemer'),
					'content' => __('Content', 'microthemer'),
					'pages' => __('Pages', 'microthemer'),
					'posts_general' => __('Posts General', 'microthemer'),
					'post_single_view' => __('Posts Single View', 'microthemer'),
					'post_navigation' => __('Post Navigation', 'microthemer'),
					'comments' => __('Comments', 'microthemer'),
					'leave_reply_form' => __('Leave Reply Form', 'microthemer'),
			...
			***/
			...
			// feather, chain, important, pie icons
			function icon_control(
				...
				// generate icon
				$icon = '<span class="'.$icon_class.'" title="'.$title.'" data-pos="'.$pos_title.'" data-neg="'.$neg_title.'"  data-input-type="'.$con.'"
			...
			*/
				...
				foreach ($this->menu as $group_key => $arr){
					...
					if (!empty($arr['sub'])){
						...
						foreach ($arr['sub'] as $item_key => $arr2){
							...
							$data_attr = !empty($arr2['data_attr']) ? 'data-'.$arr2['data_attr']['key']
								.'="'.$arr2['data_attr']['val'].'"' : '';
							...
							if (isset($arr2['input'])){
								$html.= '
								<div class="combobox-wrap input-wrap hidden">
			...
			function toggle($item_key, $arr){
				...
				$html.= '
				<div '.$id.' class="onoffswitch ui-toggle uit-par '.$on.'" data-aspect="'.$item_key.'">
			...
			function menu_section_html($section_name, $array) {
				...
				<li id="<?php echo 'strk-'.$section_name; ?>" class="section-tag strk strk-sec <?php echo $sec_class; ?>">
					...
					<span class="folder-icon tvr-icon sortable-icon" title="<?php esc_attr_e("Reorder folder", 'microthemer'); ?>"></span>
			...
			function menu_selector_html($section_name, $css_selector, $array, $sel_loop_count) {
				...
				<li id="<?php echo 'strk-'.$section_name.'-'.$css_selector; ?>" class="selector-tag strk strk-sel <?php echo $sel_class; ?>">
					...
					<div class="sel-row item-row">
						<span class="tvr-icon selector-icon sortable-icon" title="<?php esc_attr_e("Reorder selector", 'microthemer'); ?>"></span>
						...
						<span class="manage-selector-icons manage-icons">
							...
							<span class="tvr-icon hightlight-icon highlight-preview" title="<?php esc_attr_e("Highlight selector scope", 'microthemer'); ?>"></span>
						</span>
						<?php
			...
			function selector_add_modify_form($con, $tip, $labelCss = '', $section_name ='', $css_selector = '') {
				...
				<div class='<?php echo $con; ?>-selector-form float-form hidden'>
					...
					<p>
						<label><?php esc_html_e('CSS Selector Code:', 'microthemer'); ?></label>
					...
					<p class="sel-toggles">
						...
						if ($display == 'Edit') {
							$button_name = __("Edit Selector", 'microthemer');
						}
						?>
			...
			function tvr_css3_imp($section_name, $css_selector_slug, $property_group_name, $prop, $con, $mq_key) {
				if ($this->preferences['css_important'] != '1') {
					if ($con == 'mq') {
						$important_val = $this->options['non_section']['important']['m_query'][$mq_key][$section_name][$css_selector_slug][$property_group_name][$prop];
					} else {
						$important_val = $this->options['non_section']['important'][$section_name][$css_selector_slug][$property_group_name][$prop];
					}
					if ($important_val == '1') {
...
';
			...
			function filter_incoming_data($con, $data){
				...
				// Unitless css values may need to be auto-adjusted, incluidng MQs
			...
			function load_json_file($json_file, $theme_name, $context = '') {
				...
				if (file_exists($json_file)) {
					...
					if (!$read_file = fopen($json_file, 'r')) {
						$this->log(
							esc_html__('Read json error', 'microthemer'),
							'<p>' . sprintf( esc_html__('WordPress was not able to read %s.', 'microthemer'), $this->root_rel($json_file) )
							. '. '.$this->permissionshelp.'</p>'
						);
					}
					else {
					...
					if (!$json_error) {
						...
						if ($context == __('Merge', 'microthemer')) {
						...
						// updates options var and save settings in db
				...
				// the config file doesn't exist
					...
					$this->log(
						esc_html__('Config file missing', 'microthemer'),
						'<p>' . sprintf(
							esc_html__('%1$s settings file could not be loaded because it doesn\'t exist in %2$s', 'microthemer'),
		...
		class tvr_microthemer_frontend {
			...
			*/
			...
			function __construct(){
				...
				if (empty($this->preferences['admin_bar_shortcut']) or $this->preferences['admin_bar_shortcut'] == 1) {
					add_action( 'admin_bar_menu', array(&$this, 'custom_toolbar_link'), 999999);
				}
